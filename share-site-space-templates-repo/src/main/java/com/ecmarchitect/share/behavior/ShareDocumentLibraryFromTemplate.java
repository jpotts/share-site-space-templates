...
public class ShareDocumentLibraryFromTemplate implements NodeServicePolicies.OnCreateNodePolicy {
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef) {
        ...
        // if the spaceTemplate has a rules folder, copy that to the new documentLibrary
        copyRules(spaceTemplate, documentLibrary);
    }

    private void copyRules(NodeRef source, NodeRef target) {
        NodeRef ruleFolder = getRuleFolder(source);
        if (ruleFolder == null) {
            return;
        }

        copyService.copy(ruleFolder, target, RuleModel.ASSOC_RULE_FOLDER, RuleModel.ASSOC_RULE_FOLDER, true);
    }
